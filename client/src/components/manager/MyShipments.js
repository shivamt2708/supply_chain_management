...
const MyShipments = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(`http://localhost:4000/manager/my-shipments?sender_email=${username}`);
  ...
  const handleOnChange = async (para1, para2) => {
    try{
      const response = await axios.put(`http://localhost:4000/manager/action?shipment_id=${para1}`);
    }
    catch (error) {
      console.error(error);
